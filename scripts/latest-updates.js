(function () {
  ...
  function updateDueDateStatuses() {
    ...
    document.querySelectorAll('.due-date-card').forEach(function (card) {
      ...
      const dueDate = parseCardDate(card);

      if (/^Advance Tax – Q[23]\b/.test(title)) {
        advanceCards.push({ card, title, dueDate });
        return;
      }
      ...
      if (isToday || isOverdue) {
        ...
        notice.querySelector('strong').textContent = isToday ? '⚠️ Due Today' : '⚠️ Overdue';
        card.setAttribute('data-due-status', isToday ? 'today' : 'overdue');
      }
    });

    // The public panel intentionally shows only the current Q2/Q3 advance-tax
    // deadline: Q2 through 15 September, then Q3 through 15 December.
    if (advanceCards.length) {
      let selected = advanceCards.find(entry => entry.dueDate && entry.dueDate >= today);
      if (!selected) selected = advanceCards[0];

      advanceCards.forEach(function (entry) {
        entry.card.hidden = entry !== selected;
        if (entry === selected && entry.dueDate) {
          let notice = entry.card.querySelector('.urgent-notice');
          ...
          if (isToday) {
            if (!notice) {
              ...
              notice.style.padding = '0.6rem';
              notice.appendChild(document.createElement('strong'));
              entry.card.appendChild(notice);
            }
            notice.querySelector('strong').textContent = '⚠️ Due Today';
            entry.card.setAttribute('data-due-status', 'today');
          } else if (notice) {
            notice.hidden = true;
          }
        }
  ...
  function buildTicker(items) {
    ...
    if (!safeItems.length) { section.hidden = true; return; }
    const makeLink = item => {
      const a = document.createElement('a');
      a.className = 'latest-update-link'; a.href = item.url; a.target = '_blank'; a.rel = 'noopener noreferrer';
      const title = document.createElement('span'); title.textContent = item.title; a.appendChild(title);
      if (item.source) { const source = document.createElement('span'); source.className = 'latest-update-source'; source.textContent = item.source; a.appendChild(source); }
      a.setAttribute('aria-label', `${item.title} — ${item.source || 'Official source'}`); return a;
    };
  ...
  function addFallbackAnchorBehaviour() {
    ...
    if (!link) return;
    link.addEventListener('click', function () { const section = document.getElementById(SECTION_ID); if (section) section.scrollIntoView({ behavior: 'smooth', block: 'start' }); });
  }

  function addClientPortalEntry() {
    const nav = document.querySelector('.nav ul');
    if (!nav || nav.querySelector('[data-client-portal-preview]')) return;
    const item = document.createElement('li'); item.setAttribute('data-client-portal-preview', 'true');
    const link = document.createElement('a'); link.href = 'client-portal.html'; link.className = 'client-portal-login'; link.textContent = 'Client Login'; link.setAttribute('aria-label', 'KKA Client Portal — Coming Soon');
    item.appendChild(link); nav.appendChild(item);
    if (!document.getElementById('client-portal-nav-styles')) {
      const style = document.createElement('style'); style.id = 'client-portal-nav-styles'; style.textContent = `
  ...
  function addLegalFooterLinks() {
    ...
    const legal = document.createElement('div'); legal.setAttribute('data-kka-legal-links', 'true'); legal.className = 'kka-legal-links';
    legal.innerHTML = `<a href="privacy-policy.html">Privacy Policy</a><span aria-hidden="true">|</span><a href="terms-of-use.html">Terms of Use</a><span aria-hidden="true">|</span><a href="disclaimer.html">Disclaimer</a>`;
    footerBottom.appendChild(legal);
    if (!document.getElementById('kka-legal-links-styles')) {
      const style = document.createElement('style'); style.id = 'kka-legal-links-styles'; style.textContent = `.kka-legal-links{display:flex;justify-content:center;align-items:center;gap:.55rem;flex-wrap:wrap;margin-top:.7rem;font-size:.84rem}.kka-legal-links a{color:inherit;text-decoration:underline;text-underline-offset:2px;opacity:.9}.kka-legal-links a:hover{opacity:1}`; document.head.appendChild(style);
    }
  }

  async function init() {
    updateFooterDate(); injectStyles(); addFallbackAnchorBehaviour(); addClientPortalEntry(); addLegalFooterLinks(); updateDueDateStatuses();
    const adminLogin = document.querySelector('.admin-login'); if (adminLogin) adminLogin.href = 'https://files.ca-kka.com/';
    try {
      const response = await fetch(DATA_URL, { cache: 'no-store' });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const data = await response.json(); buildTicker(data.items);
    } catch (error) { const section = document.getElementById(SECTION_ID); if (section) section.hidden = true; console.warn('Latest Updates unavailable; section hidden.', error); }
  }
