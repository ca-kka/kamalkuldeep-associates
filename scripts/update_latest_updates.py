...
def income_tax_items(url: str):
    ...
    for text_node in soup.find_all(string=date_re):
        date_match = date_re.search(str(text_node))
        if not date_match:
            continue
        ...
        for _ in range(6):
            ...
            for anchor in container.find_all('a', href=True):
                ...
                if not valid_url(href) or len(title) < 20 or len(title) > 240:
                    continue
                if title.lower() in {'click here', 'read more', 'view all updates'}:
                    continue
...
def icai_items(url: str):
    soup = BeautifulSoup(get(url), 'html.parser')
    items = []
    date_re = re.compile(r'\((\d{2}-\d{2}-\d{4}|\d{2}-[A-Za-z]{3}-\d{4})\)\s*\.?$')
