...
Deno.serve(async req => {
  ...
  try {
    ...
    // Idempotency: if a previous attempt already created the document, never
    // report a failure just because the client retried the completion request.
    ...
    if (existingDocument) {
      ...
      return json({
        state: existingDocument.status,
        documentId: existingDocument.id,
        classification: {
          clientId: existingDocument.client_id,
          area: existingDocument.area,
          financialYear: existingDocument.financial_year,
          period: existingDocument.filing_period,
          confidence: existingDocument.classification_confidence,
        },
        oneDrive: null,
        oneDriveRegistryLinked: true,
        requiresReview: existingDocument.status === "review",
    ...
    const finalPath = manual
      ? `${upload.proposed_client_id}/${upload.proposed_area}${folderFy}${folderPeriod}/${crypto.randomUUID()}`
      : (upload.proposed_client_id ? `${upload.proposed_client_id}/${crypto.randomUUID()}` : `review/${crypto.randomUUID()}`);
    ...
    if (manual && upload.proposed_client_id && upload.proposed_area) {
      ...
      const r = await fetch(`${SUPABASE_URL}/functions/v1/onedrive-storage`, {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          clientId: upload.proposed_client_id,
          area: upload.proposed_area,
          financialYear: upload.proposed_financial_year ?? null,
          filingPeriod: upload.proposed_period ?? null,
        }),
      });
      ...
      oneDrive = {
        ...
        parentDriveItemId: b.destination?.parentDriveItemId ?? null,
        registryId: b.registry?.id ?? null,
      };
    }
    ...
    const inserted = await service.from("documents").insert({
      ...
      area: upload.proposed_area,
      financial_year: upload.proposed_financial_year,
      filing_period: upload.proposed_period,
      status,
    ...
    // Registry linkage is deliberately best-effort. The document record is
    // authoritative; registry reconciliation can repair this association later.
    let registryLinkError: string | null = null;
    if (oneDrive?.registryId) {
      const { error: linkError } = await service.from("onedrive_file_registry").update({
    ...
    await service.from("audit_logs").insert({
      ...
      metadata: {
        ...
        oneDriveRegistryLinked: !registryLinkError,
    ...
    return json({
      ...
      classification: {
        ...
        period: document.filing_period,
        confidence: document.classification_confidence,
      },
      oneDrive,
      oneDriveRegistryLinked: !registryLinkError,
      oneDriveRegistryLinkError: registryLinkError,
      requiresReview: document.status === "review",
    });
  } catch (error) {
