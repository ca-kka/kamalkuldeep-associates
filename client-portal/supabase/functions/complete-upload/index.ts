...
Deno.serve(async req=>{
  ...
  try{
    ...
    if(oneDrive?.registryId){const{error:linkError}=await service.from("onedrive_file_registry").update({document_id:document.id,deleted_at:null,deletion_source:null,classification_status:"classified",updated_at:new Date().toISOString()}).eq("id",oneDrive.registryId);if(linkError)throw new Error("The file was uploaded to OneDrive, but KKA could not link the storage record to the document.");}
    await service.from("document_uploads").update({completed_at:new Date().toISOString()}).eq("id",upload.id);await service.from("audit_logs").insert({actor_id:user.id,client_id:document.client_id,action:"document_uploaded",entity_type:"document",entity_id:document.id,metadata:{filename:upload.original_filename,state:document.status,confidence:upload.confidence,manual,oneDrive:oneDrive??null}});
    return json({state:document.status,documentId:document.id,classification:{clientId:document.client_id,area:document.area,financialYear:document.financial_year,period:document.filing_period,confidence:document.classification_confidence},oneDrive,requiresReview:document.status==="review"});
