...
Deno.serve(async (req:Request)=>{
  ...
  try{
    ...
    const anon=createClient(supabaseUrl,Deno.env.get("SUPABASE_ANON_KEY")!,{auth:{autoRefreshToken:false,persistSession:false}});
    ...
    if(!isStaff){
      ...
      const {data:membership,error:membershipError}=await service.from("client_memberships")
        .select("client_id").eq("user_id",user.id).eq("client_id",doc.client_id).maybeSingle();
      if(membershipError) throw membershipError;
      if(!membership?.client_id) return errorResponse("Document access denied.",403);
    }

    const {data:file,error:fileError}=await service.storage.from("client-documents").download(doc.storage_path);
    if(fileError||!file) {
    ...
    headers.set("Content-Disposition",`inline; filename="${safeFilename(doc.original_filename)}"`);
    ...
    return errorResponse("The document could not be opened.",500);
  }
});