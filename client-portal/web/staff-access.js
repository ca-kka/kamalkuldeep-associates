...
export async function setupStaffAccess({ root, supabase, currentProfile, currentUser, esc }) {
  ...
  async function call(action, payload = {}) {
    ...
    const response = await fetch(`${SUPABASE_URL}/functions/v1/manage-staff-account-v1`, { method:"POST", headers:{"Content-Type":"application/json",Authorization:`Bearer ${session.access_token}`}, body:JSON.stringify({action,...payload}) });
  ...
  async function load() {
    ...
    try {
      ...
      rows.innerHTML = profiles.map(p => `<tr><td><strong>${esc(p.full_name||"Unnamed")}</strong><small>${p.id===currentUser?.id?"Current account":""}</small></td><td>${esc(p.email||"—")}</td><td><span class="pill ${p.role==="admin"?"success":"neutral"}">${p.role==="admin"?"Administrator":p.role==="staff"?"Staff":esc(p.role)}</span></td><td><span class="pill ${p.active?"success":"neutral"}">${p.active?"Active":"Inactive"}</span></td><td><button class="secondary table-action" data-manage-staff="${esc(p.id)}" ${p.id===currentUser?.id?'disabled title="The current administrator cannot be changed here"':''}>Manage</button></td></tr>`).join("") || `<tr><td colspan="5">No staff profiles found.</td></tr>`;
